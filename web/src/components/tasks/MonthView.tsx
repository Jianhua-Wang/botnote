      ...
      <div className="flex-1 grid grid-cols-7 grid-rows-6 bg-line/30 gap-px overflow-y-auto scrollbar-thin">
        {days.map((d) => {
          ...
          return (
            <div
              key={d.toISOString()}
              className={`group bg-surface p-1 min-h-[96px] ${
                outOfMonth ? "opacity-50" : ""
              } ${today ? "ring-1 ring-accent ring-inset bg-accentSoft/30" : ""}`}
            >
              <div className="flex items-center justify-between mb-1 px-1">
                ...
                <button
                  className="text-xxs text-faint hover:text-accent opacity-0 group-hover:opacity-100 transition-opacity"
              ...
              <div className="space-y-0.5">
                ...
                {tasks.length > 4 && (
                  <div className="text-xxs text-muted pl-1">+{tasks.length - 4}</div>
                )}
              </div>
            </div>
          );
